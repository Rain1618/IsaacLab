...
def init_to_motion(env, clip, phase_t, device):
    ...
    # Init shadow robot if it exists
    if "shadow_robot" in env.unwrapped.scene.keys():
        shadow = env.unwrapped.scene["shadow_robot"]
        shadow.write_root_state_to_sim(robot.data.root_state_w.clone())
        shadow.write_joint_state_to_sim(q_t, v_t, joint_ids=jids)
...
def collect_bc_data(env, clip, device):
    ...
    for step in range(n_steps):
        ...
        update_shadow_robot(env, clip, phase_t, device)
